...
<Callout type="info" title="Retrieval depends on the backing store">
  The shipped CCR backend is in-memory with a default 5 minute TTL. It is process-local and does not survive restarts. Durable SQLite/Redis-style CCR backends are roadmap work, not shipped first-party backends in this repository.
</Callout>
...
| SQLite | Roadmap / third-party only | Not guaranteed by this repo | No first-party SQLite CCR backend is currently shipped. |
| Redis | Roadmap / third-party only | Not guaranteed by this repo | No first-party Redis CCR backend is currently shipped. |

For local development, the default in-memory store is enough to let the model recover recently omitted content. For long-lived sessions, restart survival, or multi-worker/team proxy deployments, use or implement a durable backend and define namespace/auth policy explicitly.
