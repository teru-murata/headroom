...
| OpenAI | Preserve byte-identical prefixes for automatic caching | Up to 50% on cached tokens |
| Google | `CachedContent` API | Up to 75% on cached tokens |

## CCR: Compress-Cache-Retrieve

When SmartCrusher compresses a tool output or eligible context is omitted, the original content is stored in the CCR compression cache. If the LLM needs the full data while the cache entry is still available, it can request retrieval via a `ccr_retrieve` tool call. This makes compression retrievable within the active store's retention guarantees.

```
Compress:  1000 items  ->  15 items  (stored original in CCR)
Cache:     Hash-indexed in-memory store by default (5 minute TTL)
Retrieve:  LLM calls ccr_retrieve("abc123")  ->  original 1000 items
```

The default backend is `InMemoryBackend`. It is process-local and does not survive restarts; multi-worker deployments have one CCR store per worker unless a request-scoped or durable backend is installed. For local single-host restart survival, set `HEADROOM_CCR_BACKEND=sqlite`; `HEADROOM_CCR_SQLITE_PATH` can override the database path. Redis/Valkey hosted profiles, tenant authorization, and signed remote marker policy belong to coding-agent-bridge, not Headroom's local CCR backend.
...
- **System prompts and stable provider prefixes**: Content preserved by default; cache-aware rewrite/delta behavior is future explicit work
