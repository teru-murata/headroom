...
The shipped backend is `InMemoryBackend`. It is process-local and does not survive restarts; multi-worker deployments have one CCR store per worker unless a request-scoped or third-party backend is installed. `HEADROOM_CCR_BACKEND` can load external backends through the `headroom.ccr_backend` entry point, but this repository does not currently ship first-party SQLite or Redis CCR backends.
