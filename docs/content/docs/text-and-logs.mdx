...
output. Source code is preserved verbatim by default. When a preset omits
substantial output, it uses the existing local CCR marker/store flow so the
full original can be retrieved locally. Hosted/team marker policy, Redis/Valkey
profiles, and SaaS retrieve authorization remain outside Headroom's local
preset layer.
