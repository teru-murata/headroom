...
│              │     │                │     │                     │
│ Detect cache │     │ Detect type &  │     │ Score messages &    │
│ risk /       │     │ route to best  │     │ fit within token    │
│ protect zone │     │ compressor     │     │ budget              │
└──────────────┘     └────────────────┘     └─────────────────────┘
```

1. **CacheAligner** detects volatile prefix content and cache-risk zones so stable provider-prefix content can be protected by local policy.
