...
- CacheAligner detects cache-risk zones; it does not guarantee Anthropic/OpenAI prefix cache hits
- Token counting may use model-specific tokenizers or calibrated local estimates; provider billing tokens can differ
- Compression support depends on provider and integration path; protected prefix content should not be rewritten by default
