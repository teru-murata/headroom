...
Model wrapping routes eligible messages through the Headroom pipeline. Hooks
compress individual tool results. Stable conversation and provider-prefix
content should remain protected unless the selected mode or a future explicit
feature changes that policy:
...
HeadroomStrandsModel.stream()
    routes eligible messages through Headroom
    |
    v
Provider API (Bedrock, etc.)
```

The model wrapper uses the full Headroom pipeline (CacheAligner, ContentRouter,
IntelligentContext). The hook provider uses SmartCrusher directly for fast JSON
