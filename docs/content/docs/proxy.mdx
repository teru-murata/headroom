...
The Headroom proxy is a standalone HTTP server that routes eligible request
content through local compression before forwarding to the configured provider.
It is intended for local developer workflows by default; it does not implement
hosted/team bridge operation or SaaS retrieve authorization.
...
# This exposes the main proxy API on the network, but CCR retrieve routes
# remain loopback-only until shared remote proxy mode is implemented.
...
CCR retrieve routes that can expose cached originals or retrieval hashes are loopback-only.
Binding the proxy to `0.0.0.0` does not make `/v1/retrieve*` available to non-loopback clients; shared remote proxy mode needs an explicit auth and namespace policy.
...
By default, the proxy uses the shared **ContentRouter** pipeline. It routes
eligible text, logs, JSON, images, and tool outputs through the currently
enabled compressors and preserves reversible CCR markers where applicable.
Coding-agent source code and stable provider-prefix content are protected unless
the selected mode or future explicit feature says otherwise.
...
Durable local history with hourly, daily, weekly, and monthly rollups. Powers
the proxy's local `/dashboard` view when that local process is running.
...
Remote/shared proxy deployment is roadmap work, not a complete security mode.
If you bind the proxy outside loopback, add your own network access control and keep in mind that `/v1/retrieve*` stays loopback-only until the auth and namespace policy tracked in the fork roadmap is implemented.
