...
| Durable CCR backends | Local SQLite is shipped; hosted/team Redis or Valkey remains out of scope for Headroom. | [#8](https://github.com/teru-murata/headroom/issues/8) |
...
Platform support note: macOS auth reuse via Copilot CLI Keychain storage is covered by upstream smoke testing. Windows Credential Manager, Linux Secret Service / `secret-tool`, and Docker/CI token-injection paths are implemented as local auth-discovery paths, but still need real OS validation before they should be considered fully vetted. For Docker and CI, prefer passing an explicit `GITHUB_COPILOT_TOKEN` or `GITHUB_COPILOT_GITHUB_TOKEN` rather than relying on host keychain access.
